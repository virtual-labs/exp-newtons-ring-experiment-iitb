...
function activity1() {
	...
	pp.showtitle(
		`<p id="exp-title">Let's Identify the components required</span><p>`,
		3
	);
	pp.showscore(0, 3);
	canvas = pp.canvas;
...
function load_higlighted_images() {
	...
	a1_labels = [
		...
		new Chemistry.Geo_Text(
			'Convex Lens',
			new Chemistry.Point(1370, 300),
			canvas
		),

		new Chemistry.Geo_Text(
			'Monochromatic Light Source',
			new Chemistry.Point(1500, 600),
			canvas
		),
